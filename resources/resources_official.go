...
// RegisterModule registers a resource module.
func (r *ResourceRegistry) RegisterModule(module ResourceModule) {
	...
	for _, tmpl := range module.Templates() {
		r.templates[tmpl.Template.URITemplate] = tmpl
	}
}
...
// ListResourcesByCategory returns resource URIs filtered by category.
...
func (r *ResourceRegistry) RegisterWithServer(s *registry.MCPServer) {
	...
	for _, td := range r.templates {
		wrapped := r.wrapTemplateHandler(td.Template.URITemplate, td)
...
func (r *ResourceRegistry) wrapHandler(uri string, rd ResourceDefinition) ResourceHandlerFunc {
	...
	return func(ctx context.Context, request *mcp.ReadResourceRequest) (result *mcp.ReadResourceResult, err error) {
