...
Error codes: `ErrClientInit`, `ErrInvalidParam`, `ErrTimeout`, `ErrNotFound`,
`ErrAPIError`, `ErrPermission` (defined in `handler/result.go`).
...
Use adapter functions: `registry.MakeTextContent()`,
`registry.MakeErrorResult()`, `registry.ExtractArguments()` -- never
SDK-specific constructors directly.

## Parameter Extraction

```go
name := handler.GetStringParam(req, "name")
count := handler.GetIntParam(req, "count", 10)
flag := handler.GetBoolParam(req, "verbose", false)
```
...
- Write-mode tools must have `confirm_write` parameter for destructive
  operations
