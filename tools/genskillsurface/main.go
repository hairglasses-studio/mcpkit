...
func buildMirror(skillName, frontMatter, body, note string) []byte {
	...
	b.WriteString("\n")
	fmt.Fprintf(&b, "<!-- GENERATED BY hg-skill-surface-sync.sh FROM .agents/skills/%s/SKILL.md; DO NOT EDIT -->\n\n", skillName)
	b.WriteString(note)
	b.WriteString("\n\n")
	b.WriteString(body)
...
func buildAlias(alias skillAlias, body, canonicalName string) []byte {
	...
	b.WriteString("---\n\n")
	fmt.Fprintf(&b, "<!-- GENERATED BY hg-skill-surface-sync.sh FROM .agents/skills/%s/SKILL.md; DO NOT EDIT -->\n\n", canonicalName)
	fmt.Fprintf(&b, "Compatibility alias for the canonical `%s` skill.\n\n", canonicalName)
	b.WriteString(body)
	if !strings.HasSuffix(body, "\n") {
		b.WriteString("\n")
	}
