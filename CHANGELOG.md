...
- **fleetinventory — CI gate (`fleet_inventory_baseline` + `fleet_inventory_check`)** (from the SBOM/CI research, the generate/gate split grype/syft/Scorecard use): `fleet_inventory_baseline` emits a compact, committable per-repo projection of a scored scan (composite / baseline-violation count / security-finding count / Tasks-conformance) — 4.8KB for the 32-repo fleet, small enough to commit and review. `fleet_inventory_check` scans fresh and diffs against a committed baseline, returning `passed=false` (for CI exit-code) when a repo's composite drops beyond the allowed delta (default 5.0), gains a baseline violation or security finding, or becomes Tasks-shape non-conformant. New repos are reported, never failed. The "new security finding" / "new Tasks-shape non-conformance" triggers fall out of the same baseline diff — not separate mechanisms. Scan stays pure data (never fails); the gate is the separate step. Deferred follow-up (research-recommended): a dependency/vuln dimension (offline Go vuln DB snapshot via `x/mod`, version-presence matching — the concrete target is the mcpkit `mcp-go v0.54.0` < v0.56.0 DNS-rebinding pin) and `make ci` wiring in consumers.
