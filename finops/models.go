...
var DefaultPricing = map[string]Pricing{
	...
	"gpt-5.5":           {5.0, 30.0, 2.50},
	"o1":                {15.0, 60.0, 7.50},
	"o1-mini":           {3.0, 12.0, 1.50},
	"o3":                {10.0, 40.0, 5.00},
	"o3-mini":           {1.10, 4.40, 0.55},
	"o4-mini":           {1.10, 4.40, 0.55},
	"codex-mini-latest": {1.50, 6.0, 0.75},
...
func ModelCostWithCache(model string, inputTokens, outputTokens, cacheReadTokens int64) float64 {
	canonical := NormalizeModelName(model)
	p, ok := DefaultPricing[canonical]
	if !ok {
		return 0.0
	...
	return float64(inputTokens)/1_000_000*p.InputPerMToken +
		float64(outputTokens)/1_000_000*p.OutputPerMToken +
		float64(cacheReadTokens)/1_000_000*p.CacheReadPerMToken
}
